...
namespace sbtw.Editor.Overlays.Settings
{
    public class GeneralSection : SettingsSection
    {
        ...
        private class EnvironmentSettings : SettingsSubsection
        {
            ...
            [Resolved]
            private IStudioManager studioManager { get; set; }

            private Bindable<int?> portSetting;
            private Bindable<int> portConfig;

            [BackgroundDependencyLoader]
            private void load()
            {
                portConfig = config.GetBindable<int>(EditorSetting.DebugPort);
                portSetting = new Bindable<int?>(portConfig.Default) { Value = portConfig.Value };
                ...
                Children = new Drawable[]
                {
                    ...
                    {
                        LabelText = @"Debug Port",
                        Current = portSetting,
                    }
                };

                portSetting.ValueChanged += e =>
                {
                    if (e.NewValue.HasValue)
                        portConfig.Value = e.NewValue.Value;
                };
            }
        }
